...
export default function QuotePreview({ quote, onBack }: QuotePreviewProps) {
  ...
  const [factureGenereeAt, setFactureGenereeAt] = useState<string | undefined>(quote.facture_generee_at);
  const [devisModifieAt, setDevisModifieAt] = useState<string | undefined>(quote.devis_modifie_at);
  ...
  const peutGenererFacture =
    status === 'accepté' && (
      !factureGenereeAt ||
      (devisModifieAt && new Date(devisModifieAt) > new Date(factureGenereeAt))
    );
  ...
  const buildEmailHtml = (
    ...
    const lignesRows = emailItems.map((item) => `
      <tr>
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      const emailItems = (lignesData || []).map((row: any) => ({
        description: row.description ?? row.prestations?.nom ?? '',
            ...
            <div className="mb-8">
              {peutGenererFacture ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-600 text-white rounded-xl font-bold text-lg hover:bg-green-700 transition disabled:opacity-50 shadow-lg shadow-green-100"
                >
                  <Receipt className="w-6 h-6" />
                  {generatingFacture ? 'Génération en cours...' : factureGenereeAt ? 'Regénérer la facture (devis modifié)' : 'Générer la facture'}
                </button>
              ) : (
                <div className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-50 text-green-700 rounded-xl font-bold text-base border-2 border-green-200">
                  <Receipt className="w-5 h-5" />
                  Facture générée le {new Date(factureGenereeAt!).toLocaleDateString('fr-FR')}
                </div>
              )}
            </div>
          )}
